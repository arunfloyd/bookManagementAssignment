...
const Dashboard = () => {
  ...
  const AddBox = ({ title, count, link }) => (
    ...
    >
      <div className="text-3xl font-bold mb-2">{count}</div>
      <div className="text-gray-600 mb-4 text-center">{title}</div>
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          <AddBox title="Books in Library" count={counts.books} link="/book" />
          <AddBox title="Authors" count={counts.authors} link="/author" />
          <AddBox title="Languages" count={counts.languages} link="/language" />
        </div>
      </div>
    </div>
  );
